...
## std::optional
std::optional is a type that was introduced in C++ 17 that helps handle optional output from functions and some other things. It allows something to either be empty, or hold data of the given type. Here are some example declarations of variables. The empty value it refers to is std::nullopt which is null for std::optional.
```C++
