...
## Chars
Chars store a single text value. The characters must be in single quotes. They occupy one byte in memory. The max and min value of a signed char is 127 and -127. For an unsigned char it is 0 and 255. More info (https://www.learncpp.com/cpp-tutorial/chars/).
```C++
char grade{'A'}
char randomLetter{'q'}
```